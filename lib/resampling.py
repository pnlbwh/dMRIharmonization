...
def resize_spm(lowResImg, inPrefix):
    ...
    p = Popen((' ').join(
        [pjoin(SCRIPTDIR, 'spm_bspline_exec', 'bspline'), inPrefix]), shell=True)
    p.wait()

    highResImg = np.nan_to_num(
        loadmat(
            inPrefix +
            '_resampled.mat')['highResImg'])
...
def save_high_res(fileName, sp_high, lowResImgHdr, highResImg):
    ...
    save_nifti(
        fileName,
        highResImg,
        affine=imgHdrOut.get_best_affine(),
        hdr=imgHdrOut)
...
def resampling(
    ...
    lowResImgPrime, b0 = normalize_data(
        lowResImg, mask=lowResMask, where_b0=where_b0)
    ...
    if interp_toolbox == 'scipy':
        ...
        for i in np.where(bvals > B0_THRESH)[0]:
            ...
            highResImg[:, :, :, i] = resize(
                lowResImg[:, :, :, i], (sx, sy, sz), order=sOrder, mode='symmetric')
        ...
        # resample the mask
        highResMask = resize(
            lowResMask.astype('float'),
            (sx,
             sy,
             sz),
            order=1,
            mode='symmetric')  # order 1 for linear interpolation

    elif interp_toolbox == 'spm':
        step = sp_high / sp_low
        sx, sy, sz = [int(x) for x in (
            (lowResImg.shape[:3] + step + 0.01 - 1) / step + 1)]

        inPrefix = lowResImgPath.split('.nii')[0]
        # save space resolutions, image size, and bspline order
        savemat(inPrefix + '_sp.mat',
                {'sp_high': sp_high,
                 'sp_low': sp_low,
        ...
        for i in np.where(bvals > B0_THRESH)[0]:
            print('Resampling gradient ', i)
            highResImg[:, :, :, i] = resize_spm(
                lowResImg[:, :, :, i], inPrefix)
    ...
    save_high_res(
        ...
        sp_high,
        lowResMaskHdr,
        highResMask.astype('uint8'))

    # resample the b0 --------------------------------------------------------
    highResB0PathTmp = lowResImgPath.split(
        '.nii')[0] + '_resampled_bse_tmp.nii.gz'
    # using min= 1. is unnecessary
    np.nan_to_num(b0HighRes).clip(min=0., out=b0HighRes)
    ...
    p = Popen((' ').join(
        ['unring.a64', highResB0PathTmp, highResB0Path]), shell=True)
    p.wait()

    check_call(['rm', highResB0PathTmp])
    b0_gibs = load(highResB0Path).get_data()
    # using min= 1. is unnecessary
    np.nan_to_num(b0_gibs).clip(min=0., out=b0_gibs)
