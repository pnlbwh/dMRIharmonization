...
if __name__ == '__main__':
    ...
    ref_mean = list(
        0.05 *
        np.random.randn(20) +
        0.5)  # [0.46, 0.49, 0.44, 0.40]
    target_mean_before = list(
        0.05 *
        np.random.randn(20) +
        0.45)  # [0.42, 0.58, 0.43, 0.66]
    target_mean_after = list(
        0.05 *
        np.random.randn(20) +
        0.5)  # [0.5 , 0.45, 0.40, 0.55]
    labels = ['Reference', 'Target_before', 'Target_after']

    harm_plot([ref_mean, target_mean_before,
               target_mean_after], labels, '/tmp/abc')
