...
def custom_spherical_structure(D):
    ...
    [y, x, z] = np.meshgrid(np.arange(-sw, sw + 1),
                            np.arange(-sw, sw + 1), np.arange(-sw, sw + 1))
...
def approx(imgPath, maskPath):
    ...
    b0, shm_coeff, qb_model = rish(
        imgPath, maskPath, inPrefix, outPrefix, N_shm)
    B = qb_model.B
    ...
    # overriding applymask function with a nonbinary mask b0
    S_hat_dwi = applymask(S_hat, b0)
...
def ring_masking(directory, prefix, maskPath, shm_coeff, b0, qb_model, hdr):
    ...
    for i in range(0, N_shm + 1, 2):
        ...
        # for i=0, create new mask over denoisedImg, save it as
        # harmonized_{prefix}_mask
        if i == 0:
...
def reconst(imgPath, maskPath, moving, templatePath):
    ...
    harmImg, harmMask = ring_masking(
        directory, prefix, maskPath, shm_coeff, b0, qb_model, img.header)
    copyfile(inPrefix + '.bvec', harmImg.split('.nii')[0] + '.bvec')
    copyfile(inPrefix + '.bval', harmImg.split('.nii')[0] + '.bval')
