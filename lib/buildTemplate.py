    ...
    check_call(
        (' ').join(
            [
                pjoin(
                    SCRIPTDIR,
                    'antsMultivariateTemplateConstruction2_fixed_random_seed.sh'),
                '-d',
                '3',
                '-g',
                '0.2',
                ...
                '2',
                '-j',
                str(N_core) if N_core else str(N_proc),
                '-f',
                '8x4x2x1',
                '-o',
                outPrefix,
...
def dti_stat(siteName, imgs, masks, templatePath, templateHdr):
    ...
    morphed_mask = binary_opening(
        np.mean(
            maskData, axis=0) > 0.5, structure=generate_binary_structure(
            3, 1)) * 1
    morphed_mask_name = pjoin(templatePath, f'{siteName}_Mask.nii.gz')
    templateAffine = templateHdr.get_best_affine()
    save_nifti(
        morphed_mask_name,
        morphed_mask.astype('uint8'),
        templateAffine,
        templateHdr)
...
def difference_calc(refSite, targetSite, refImgs, targetImgs,
    ...
    for dm in measures:
        ...
        save_nifti(
            pjoin(
                templatePath,
                f'PercentageDiff_{dm}smooth.nii.gz'),
            np.mean(
                per_diff_smooth,
                axis=0),
            templateAffine,
