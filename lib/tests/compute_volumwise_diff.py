...
def bval_bvec_difference(imgPath_given_mat, imgPath_given_py, caselist):
    ...
    for case in caselist:
        ...
        # load matlab
        bvals_mat = np.array(read_bvals(imgPath_mat.split('.nii')[0] + '.bval'))
        bvecs_mat = np.array(read_bvecs(imgPath_mat.split('.nii')[0] + '.bvec'))
...
def dwi_difference(imgPath_given_mat, imgPath_given_py, caselist):
    ...
    for case in caselist:
        ...
        nib.Nifti1Image(diff, img_py_obj.affine, img_py_obj.header).to_filename(imgPath_mat.split('.nii')[0]+'_diff_mask.nii.gz')
